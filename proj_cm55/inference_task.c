...
* Macros
*******************************************************************************/
#define MODEL_DIM_HEIGHT_IDX                (1U)
#define MODEL_DIM_WIDTH_IDX                 (2U)
#define MODEL_DIM_CHANNEL_IDX               (3U)
#define INFERENCE_LOOP_DELAY_MS             (1U)
...
{  
    ...
    int    model_input_height     = od_input->dim[MODEL_DIM_HEIGHT_IDX];
    int    model_input_width     = od_input->dim[MODEL_DIM_WIDTH_IDX];
    int    model_input_channel = od_input->dim[MODEL_DIM_CHANNEL_IDX];
    ...
    float scale_model_to_image = max( (float)IMAGE_WIDTH / (float)model_input_width, 
                                    (float)IMAGE_HEIGHT / (float)model_input_height );
    ...
    while (1)
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(INFERENCE_LOOP_DELAY_MS));
        ...
        sem_result = xSemaphoreGive(model_semaphore);
