...
#include <arm_math.h> 
#if defined(ARM_MATH_MVEI) || defined(__ARM_FEATURE_MVE)
    #include <arm_mve.h>
#elif defined(__ARM_NEON) || defined(__ARM_NEON__)
    #include <arm_neon.h>
#else
    #warning "Vector intrinsics not supported on this architecture. Define ARM_MATH_MVEI or __ARM_NEON."
#endif
...
*
******************************************************************************/
int8_t * RGB565_to_RGB888_i8( uint32_t pixel, int8_t *rgb888_8p )
{
    /* Extract B, G, and R components from BGR565 pixel (already in BGR order) */
...
*
******************************************************************************/
void draw_image_pixel( uint16_t *pDst, uint16_t x, uint16_t y, uint16_t r, uint16_t g, uint16_t b, 
